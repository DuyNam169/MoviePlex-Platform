...
class AdminRevenueController
{
    ...
    public function dashboardData(): void
    {
        try {
            ...
            $total_tickets = (int)$this->pdo->query("SELECT SUM(num_tickets) FROM bookings WHERE status != 'cancelled' AND DATE(created_at) = CURDATE()")->fetchColumn() ?: 0;
            $total_checkins = (int)$this->pdo->query("SELECT SUM(num_tickets) FROM bookings WHERE status = 'checked_in' AND DATE(created_at) = CURDATE()")->fetchColumn() ?: 0;
            $total_revenue = (float)$this->pdo->query("SELECT SUM(total_amount) FROM bookings WHERE status != 'cancelled' AND DATE(created_at) = CURDATE()")->fetchColumn() ?: 0;
            $active_staff = (int)$this->pdo->query("SELECT COUNT(*) FROM employees WHERE status = 'active'")->fetchColumn() ?: 0;
            $locked_accounts = (int)$this->pdo->query("SELECT COUNT(*) FROM employees WHERE status = 'locked'")->fetchColumn() ?: 0;
            
            // Calculate dynamic trends today vs yesterday
            $stats_today_yesterday = $this->pdo->query("
                SELECT 
            ...
            $response['logs'] = $this->pdo->query("SELECT * FROM system_logs ORDER BY id DESC LIMIT 4")->fetchAll();
            $response['errors'] = []; 
    ...
    {
        try {
            $logs = $this->pdo->query("SELECT * FROM system_logs ORDER BY id DESC")->fetchAll(PDO::FETCH_ASSOC);
    ...
    {
        try {
            $employees = $this->pdo->query("SELECT * FROM employees ORDER BY id ASC")->fetchAll(PDO::FETCH_ASSOC);
    ...
    public function getRevenueReport(): void
    {
        try {
            ...
            $safe_start = $this->pdo->quote($startDate);
            $safe_end = $this->pdo->quote($endDate);

            $where_time = "DATE(created_at) BETWEEN $safe_start AND $safe_end";
            $where_time_b = "DATE(b.created_at) BETWEEN $safe_start AND $safe_end";

            // 1. Overall stats
            $total_revenue = (float)($this->pdo->query("SELECT SUM(total_amount) FROM bookings WHERE status != 'cancelled' AND $where_time")->fetchColumn() ?: 0);
            $total_tickets = (int)($this->pdo->query("SELECT SUM(num_tickets) FROM bookings WHERE status != 'cancelled' AND $where_time")->fetchColumn() ?: 0);
            $total_bookings = (int)($this->pdo->query("SELECT COUNT(*) FROM bookings WHERE status != 'cancelled' AND $where_time")->fetchColumn() ?: 0);

            $ticket_revenue = (float)($this->pdo->query("SELECT SUM(subtotal) FROM bookings WHERE status != 'cancelled' AND $where_time")->fetchColumn() ?: 0);
            $snack_revenue = (float)($this->pdo->query("SELECT SUM(GREATEST(0, total_amount - subtotal + discount)) FROM bookings WHERE status != 'cancelled' AND $where_time")->fetchColumn() ?: 0);

            // Online stats
            $online_revenue = (float)($this->pdo->query("SELECT SUM(total_amount) FROM bookings WHERE status != 'cancelled' AND payment_method != 'cash' AND $where_time")->fetchColumn() ?: 0);
            $online_tickets = (int)($this->pdo->query("SELECT SUM(num_tickets) FROM bookings WHERE status != 'cancelled' AND payment_method != 'cash' AND $where_time")->fetchColumn() ?: 0);
            $online_bookings = (int)($this->pdo->query("SELECT COUNT(*) FROM bookings WHERE status != 'cancelled' AND payment_method != 'cash' AND $where_time")->fetchColumn() ?: 0);
            $online_ticket_revenue = (float)($this->pdo->query("SELECT SUM(subtotal) FROM bookings WHERE status != 'cancelled' AND payment_method != 'cash' AND $where_time")->fetchColumn() ?: 0);
            $online_snack_revenue = (float)($this->pdo->query("SELECT SUM(GREATEST(0, total_amount - subtotal + discount)) FROM bookings WHERE status != 'cancelled' AND payment_method != 'cash' AND $where_time")->fetchColumn() ?: 0);

            // Direct / Counter stats
            $direct_revenue = (float)($this->pdo->query("SELECT SUM(total_amount) FROM bookings WHERE status != 'cancelled' AND payment_method = 'cash' AND $where_time")->fetchColumn() ?: 0);
            $direct_tickets = (int)($this->pdo->query("SELECT SUM(num_tickets) FROM bookings WHERE status != 'cancelled' AND payment_method = 'cash' AND $where_time")->fetchColumn() ?: 0);
            $direct_bookings = (int)($this->pdo->query("SELECT COUNT(*) FROM bookings WHERE status != 'cancelled' AND payment_method = 'cash' AND $where_time")->fetchColumn() ?: 0);
            $direct_ticket_revenue = (float)($this->pdo->query("SELECT SUM(subtotal) FROM bookings WHERE status != 'cancelled' AND payment_method = 'cash' AND $where_time")->fetchColumn() ?: 0);
            $direct_snack_revenue = (float)($this->pdo->query("SELECT SUM(GREATEST(0, total_amount - subtotal + discount)) FROM bookings WHERE status != 'cancelled' AND payment_method = 'cash' AND $where_time")->fetchColumn() ?: 0);

            // 2. Revenue by Movie
            $movie_revenue = $this->pdo->query("
            ...
            // 4. Detailed Bookings
            $detailed_bookings = $this->pdo->query("
