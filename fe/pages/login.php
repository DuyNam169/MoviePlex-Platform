...
.bg-overlay{
  ...
  background:linear-gradient(135deg,rgba(5,13,26,.92) 0%,rgba(5,13,26,.6) 50%,rgba(5,13,26,.82) 100%);
}
/* Floating particles */
.particles{position:absolute;inset:0;z-index:2;overflow:hidden}
.dot{
  position:absolute;width:3px;height:3px;background:rgba(255,255,255,.4);
...
.left-content{
  ...
  justify-content:space-between;padding:36px 40px;
  overflow:hidden;
}
...
.brand-icon{
  ...
  font-size:22px;color:#fff;box-shadow:0 8px 24px rgba(59,130,246,.5);
}
...
.hero-label{
  ...
  color:#93C5FD;font-size:12px;font-weight:700;padding:6px 14px;
  border-radius:20px;letter-spacing:.6px;margin-bottom:24px;
  backdrop-filter:blur(8px);
}
.hero-title{
  font-size:40px;font-weight:800;color:#fff;line-height:1.1;
...
.strip-card{
  ...
  box-shadow:0 6px 18px rgba(0,0,0,.5);transition:transform .3s;cursor:pointer;
}
...
.strip-card .rating{
  ...
  color:#FBBF24;font-size:11px;font-weight:700;padding:2px 7px;
...
.stat-val span{
  font-size:14px;background:linear-gradient(90deg,#60A5FA,#A78BFA);
  -webkit-background-clip:text;-webkit-text-fill-color:transparent;
  background-clip:text;
}
...
.iw input{
  ...
  background:#FAFBFE;
  outline:none;transition:all .25s;font-weight:500;
}
...
/* Submit btn */
.btn{
  width:100%;height:48px;border:none;border-radius:10px;font-size:14px;
  font-weight:700;font-family:inherit;cursor:pointer;transition:all .25s;
  display:flex;align-items:center;justify-content:center;gap:10px;
  background:linear-gradient(135deg,#3B82F6 0%,#6366F1 100%);
  color:#fff;box-shadow:0 6px 20px rgba(59,130,246,.3);
  position:relative;overflow:hidden;margin-bottom:20px;z-index:1;
}
.btn::before{
...
/* Divider */
.div-line{display:flex;align-items:center;gap:12px;font-size:11px;color:#94A3B8;margin-bottom:14px;font-weight:600;position:relative;z-index:1;text-transform:uppercase;letter-spacing:.5px}
.div-line::before,.div-line::after{content:'';flex:1;height:1px;background:#E2E8F0}

/* Social */
.socials{display:flex;gap:10px;margin-bottom:20px;position:relative;z-index:1}
.soc{
  flex:1;height:42px;display:flex;align-items:center;justify-content:center;gap:8px;
  background:#fff;border:1.5px solid #E2E8F0;
  border-radius:10px;font-size:13px;font-weight:600;color:#64748B;cursor:not-allowed;opacity:.6;
  position:relative;text-decoration:none;
}
.soc img{width:18px;height:18px}
.soc .tag{
  position:absolute;top:-8px;right:4px;background:#94A3B8;color:#fff;
  font-size:9px;font-weight:800;padding:2px 6px;border-radius:5px;letter-spacing:.3px;
}
...
<div class="right">
  ...
  <div id="vl">
    ...
    <!-- <div class="div-line">Hoặc tiếp tục với</div>
    <div class="socials">
      <a href="#" class="soc" onclick="return false"><span class="tag">SỚM</span><img src="https://upload.wikimedia.org/wikipedia/commons/5/53/Google_%22G%22_Logo.svg" alt="">Google</a>
      <a href="#" class="soc" onclick="return false"><span class="tag">SỚM</span><img src="https://upload.wikimedia.org/wikipedia/commons/5/51/Facebook_f_logo_%282019%29.svg" alt="">Facebook</a>
    </div> -->
...
<script>
// ── Auto-switch to register if coming from guest page ──
...
function fe_(inp,fe,msg){
  ...
  const e=document.getElementById(fe);if(e){e.textContent=msg;e.classList.add('show');}
}
function setLoad(id,on){const b=document.getElementById(id);b.classList.toggle('loading',on);b.disabled=on;}
...
document.getElementById('fl').addEventListener('submit',async function(e){
  ...
  try{
    ...
    if(d.success){alert_('la','success',d.message||'Đăng nhập thành công!');setTimeout(()=>location.href=d.redirect||'home.php',800);}
    else{alert_('la','error',d.message);setLoad('bl',false);}
  }catch{alert_('la','error','Lỗi kết nối. Vui lòng thử lại.');setLoad('bl',false);}
});
...
document.getElementById('fr').addEventListener('submit',async function(e){
  ...
  if(!ok)return;
  setLoad('br',true);
  const fd=new FormData(this);fd.append('action','register');
  try{
    const r=await fetch('auth.php',{method:'POST',body:fd});
    const d=await r.json();
    if(d.success){alert_('ra','success',d.message);this.reset();setTimeout(()=>sw('l'),1800);}
    else{alert_('ra','error',d.message);}
  }catch{alert_('ra','error','Lỗi kết nối. Vui lòng thử lại.');}
  finally{setLoad('br',false);}
});
</script>
</body>
</html>
