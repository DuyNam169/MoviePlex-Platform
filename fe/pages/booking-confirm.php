...
<title>Xác nhận đặt vé - MovieFlex</title>
...
.btn-blue:hover{background:#C21025}
...
.error-box{background:var(--card);border-radius:20px;box-shadow:0 4px 32px rgba(15,23,42,.10);padding:60px 32px;text-align:center;margin-bottom:20px}
...
.inv-meta h2{font-size:18px;font-weight:800;margin-bottom:4px}
...
<div class="page">
  <div class="container" id="main-container">
    ...
    <div class="success-box">
      <div style="background:#e2e8f0;padding:36px;text-align:center">
        <div class="skeleton" style="width:72px;height:72px;border-radius:50%;margin:0 auto 16px"></div>
        <div class="skeleton" style="height:24px;border-radius:4px;width:50%;margin:0 auto 8px"></div>
        <div class="skeleton" style="height:14px;border-radius:4px;width:40%;margin:0 auto"></div>
      </div>
    </div>
    <div class="ticket">
      <div class="ticket-top">
        <div class="skeleton" style="width:80px;height:112px;border-radius:10px;flex-shrink:0"></div>
        <div style="flex:1;display:flex;flex-direction:column;gap:10px;padding-top:4px">
          ...
          <div class="skeleton" style="height:13px;border-radius:4px;width:60%"></div>
...
<!-- INVOICE MODAL (ngoài .page để print đúng) -->
<div class="inv-overlay" id="inv-overlay" onclick="closeInvOverlay(event)">
...
<script>
const BOOKING_CODE = '<?= $codeEscaped ?>';
const USER_ROLE    = '<?= htmlspecialchars($userRole, ENT_QUOTES, 'UTF-8') ?>';
const API          = '/be/api.php';
const PAY_LABELS   = {momo:'MoMo',vnpay:'VNPay',zalopay:'ZaloPay',cash:'Tiền mặt',napas:'Napas'};
...
function renderPage(data) {
  const { booking, seats, ticketCodesMap, totalSubtotal, totalDiscount, totalAmount, relatedBookings } = data;

  const poster = booking.poster_url
    ? `<img class="ticket-poster" src="${escHtml(booking.poster_url)}" alt="">`
    : `<div class="ticket-poster-ph"><i class="fa-solid fa-film"></i></div>`;

  const payLabel = PAY_LABELS[booking.payment_method] || booking.payment_method;
  const showDate = fmtDate(booking.show_date);
  const startTime = (booking.start_time || '').substring(0, 5);
  const hallName  = escHtml(booking.hall_name || 'Phòng chiếu 1');

  // Seat rows with per-seat ticket code
  const seatRows = seats.map(s => {
    const tcode = ticketCodesMap[s] || booking.booking_code;
    return `
      <div style="display:flex;align-items:center;justify-content:space-between;border-bottom:1px dashed var(--border);padding-bottom:10px;width:100%">
        <div style="display:flex;align-items:center;gap:12px">
          <span class="seat-chip" style="background:var(--blue);color:#fff;font-size:13px;padding:4px 10px;border-radius:6px;font-weight:800">${escHtml(s)}</span>
          <span style="font-family:monospace;font-size:14.5px;font-weight:800;color:var(--text);letter-spacing:1px">${escHtml(tcode)}</span>
        </div>
        <div style="display:flex;align-items:center;gap:8px">
          <span style="font-size:11px;padding:4px 10px;border-radius:12px;font-weight:700;background:#ECFDF5;color:#10B981;border:1px solid #A7F3D0;display:inline-flex;align-items:center;gap:4px">
            <i class="fa-solid fa-circle-check"></i> Có hiệu lực
          </span>
          <div style="width:28px;height:28px;border:1.5px solid var(--border);border-radius:6px;display:flex;align-items:center;justify-content:center;background:var(--bg);color:var(--muted)">
  ...
  const discountRow = totalDiscount > 0
    ? `<div class="price-row">
        <span class="pr-label">Giảm giá (${escHtml(booking.voucher_code || '')})</span>
        <span class="pr-val pr-green">−${fmtMoney(totalDiscount)}</span>
       </div>`
    : '';

  const isStaff = USER_ROLE === 'staff';
    ...
    <div class="ticket">
      <div class="ticket-top">
        ...
        <div class="ticket-info">
          <h2>${escHtml(booking.title)}</h2>
          <div class="ticket-meta">
            <div class="tm-item"><label>Ngày chiếu</label><span>${showDate}</span></div>
            <div class="tm-item"><label>Giờ chiếu</label><span>${startTime}</span></div>
            <div class="tm-item"><label>Định dạng & Phòng</label><span>${escHtml(booking.format)} · ${escHtml(booking.subtitle_type)} · ${hallName}</span></div>
            <div class="tm-item"><label>Rạp chiếu</label><span>${escHtml(booking.cinema_name)}</span></div>
            <div class="tm-item"><label>Khách hàng</label><span>${escHtml(booking.full_name)}</span></div>
            <div class="tm-item"><label>Thanh toán</label><span>${escHtml(payLabel)}</span></div>
          </div>
        </div>
        <div class="tear"></div>
      </div>
      <div style="padding:24px">
        ...
        <div style="display:flex;flex-direction:column;gap:10px;width:100%">${seatRows}</div>
      </div>
    </div>

    <!-- PRICE -->
    <div class="price-box">
      <div class="price-row"><span class="pr-label">Vé phim (${seats.length} ghế)</span><span class="pr-val">${fmtMoney(totalSubtotal)}</span></div>
      ${discountRow}
      <div class="pr-total"><span>Tổng đã thanh toán</span><span class="pr-total-price">${fmtMoney(totalAmount)}</span></div>
    </div>

    <!-- ACTIONS -->
    <div class="actions">
      <button class="btn btn-print" onclick="openInvoice()"><i class="fa-solid fa-file-invoice"></i> In hóa đơn</button>
      ${actionBtns}
    </div>`;

  // Build invoice content
  buildInvoice(booking, seats, ticketCodesMap, totalSubtotal, totalDiscount, totalAmount);
}

function buildInvoice(booking, seats, ticketCodesMap, totalSubtotal, totalDiscount, totalAmount) {
  const payLabel  = PAY_LABELS[booking.payment_method] || booking.payment_method;
  const showDate  = fmtDate(booking.show_date);
  const startTime = (booking.start_time || '').substring(0, 5);
  const now       = new Date().toLocaleString('vi-VN');
  const seatList  = seats.join(', ');
  const pricePerSeat = seats.length > 0 ? fmtMoney(totalSubtotal / seats.length) : '—';

  const discountRow = totalDiscount > 0
    ? `<tr><td>Khuyến mãi (Mã: ${escHtml(booking.voucher_code || '')})</td><td>—</td><td class="ta-r" style="color:#16A34A">−${fmtMoney(totalDiscount)}</td></tr>`
    : '';

  const snackTotal = totalAmount - totalSubtotal + totalDiscount;
  const snackRow = snackTotal > 0
    ? `<tr><td>Bắp nước & dịch vụ cộng thêm</td><td>—</td><td class="ta-r">${fmtMoney(snackTotal)}</td></tr>`
    : '';
  ...
  document.getElementById('inv-body').innerHTML = `
    <div class="inv-header">
      ...
      <div class="inv-meta">
        ...
        <p>Ngày xuất: ${now}</p>
        <div class="inv-code-badge">${escHtml(booking.booking_code)}</div>
      </div>
    </div>
    ...
    <div class="inv-section">
      ...
      <div class="inv-grid">
        <div class="inv-field"><label>Họ và tên</label><span>${escHtml(booking.full_name)}</span></div>
        <div class="inv-field"><label>Email</label><span>${escHtml(booking.email)}</span></div>
        <div class="inv-field"><label>Số điện thoại</label><span>${escHtml(booking.phone || '—')}</span></div>
        <div class="inv-field"><label>Mã giao dịch</label><span>${escHtml(booking.transaction_id || 'N/A')}</span></div>
        <div class="inv-field"><label>Thời gian đặt</label><span>${fmtDateTime(booking.created_at)}</span></div>
        <div class="inv-field"><label>Phương thức TT</label><span>${escHtml(payLabel)}</span></div>
      </div>
    </div>
    ...
    <div class="inv-section">
      ...
      <div class="inv-grid">
        <div class="inv-field"><label>Tên phim</label><span>${escHtml(booking.title)}</span></div>
        <div class="inv-field"><label>Thời lượng</label><span>${escHtml(String(booking.duration_min || '—'))} phút</span></div>
        <div class="inv-field"><label>Định dạng</label><span>${escHtml(booking.format)} · ${escHtml(booking.subtitle_type)}</span></div>
        <div class="inv-field"><label>Phòng chiếu</label><span>${escHtml(booking.hall_name || 'Phòng chiếu tiêu chuẩn')}</span></div>
        <div class="inv-field"><label>Ngày chiếu</label><span>${showDate}</span></div>
        <div class="inv-field"><label>Giờ chiếu</label><span>${startTime}</span></div>
        <div class="inv-field" style="grid-column:1/-1">
          <label>Rạp chiếu</label>
          <span><strong>${escHtml(booking.cinema_name)}</strong> — Hotline: ${escHtml(booking.cinema_phone || '1900 6017')}<br>
            <span style="font-weight:normal;color:#64748B">${escHtml(booking.cinema_address || '')}</span></span>
        </div>
        <div class="inv-field" style="grid-column:1/-1"><label>Ghế ngồi</label><span>${escHtml(seatList)}</span></div>
      </div>
    </div>
    ...
    <div class="inv-section">
      ...
      <table class="inv-table">
        <thead>
          <tr><th>Mô tả</th><th>Số lượng / Đơn giá</th><th class="ta-r">Thành tiền</th></tr>
        </thead>
        <tbody>
          <tr>
            <td>Vé xem phim — ${escHtml(booking.title)}<br>
              <span style="font-size:11px;color:#64748B">Ghế: ${escHtml(seatList)}</span></td>
            <td>${seats.length} vé<br><span style="font-size:11px;color:#64748B">${pricePerSeat}/vé</span></td>
            <td class="ta-r">${fmtMoney(totalSubtotal)}</td>
          </tr>
          ${discountRow}
          ${snackRow}
          <tr style="border-top:1.5px dashed var(--border);border-bottom:none">
            ...
            <td class="ta-r" style="font-size:13px;padding-bottom:4px">${fmtMoney(totalAmount)}</td>
          </tr>
          ...
          <tr class="inv-total-row">
            <td colspan="2"><strong>Tổng cộng đã thanh toán</strong></td>
            <td class="ta-r"><strong>${fmtMoney(totalAmount)}</strong></td>
          </tr>
        </tbody>
      </table>
    </div>
    ...
    <div class="inv-footer">
      ...
      <p>Vui lòng xuất trình mã đặt vé <strong>${escHtml(booking.booking_code)}</strong> khi đến rạp.</p>
      <p style="color:var(--muted);font-size:11px;margin-top:6px">Hóa đơn này được tạo tự động bởi hệ thống MovieFlex vào lúc ${now}</p>
    </div>`;
}
...
function renderError(message) {
  document.getElementById('main-container').innerHTML = `
    <div class="error-box">
      ...
      <h2 style="font-size:18px;font-weight:700;margin-bottom:8px">Không thể tải thông tin đặt vé</h2>
      <p style="color:#64748B;margin-bottom:20px">${escHtml(message)}</p>
      <a href="home.php" style="background:var(--blue);color:#fff;padding:10px 24px;border-radius:10px;font-weight:700;text-decoration:none;display:inline-flex;align-items:center;gap:6px">
...
async function loadConfirmation() {
  try {
    const res = await fetch(`${API}?action=booking_confirm&code=${encodeURIComponent(BOOKING_CODE)}`);
